...
from nativeedge_common_sdk.oxy import resolve_internal_proxy

from fabric_plugin._compat import (
    ctx,
    utils,
    ctx_wrappers,
    exceptions,
...
def get_proxy_host(host_ip, port, proxy_settings=None):
    proxy_settings = proxy_settings or {}

    host_ip, port, proxy_url, _ = resolve_internal_proxy(
        proxy_settings,
        host_ip,
        port,
        ctx.deployment.id
...
def put_host(fabric_env, host_ip=None, proxy_settings=None):
    ...
    if fabric_env.get('host'):
        host_value = fabric_env.get('host')
    elif fabric_env.get('host_string'):
        host_value = fabric_env.pop('host_string')
    elif host_ip:
        host_value = host_ip
    else:
        host_value = None
    ...
    host_ip, port, proxy_url = get_proxy_host(
        host_value,
        port,
        proxy_settings
    )
    ...
    fabric_env['proxy_url'] = proxy_url
...
def _parse_proxy_url(proxy_url: str):
    ...
    # If scheme missing, urlparse treats it as path; prefix with
    if "://" not in proxy_url:
    ...
    if not host:
        raise NonRecoverableError(f"Invalid proxy_url: {proxy_url!r}")

    # Default SOCKS port if not provided
    if port is None:
        port = 1080
...
def put_proxy(fabric_env, connect_kwargs, target_host, target_port):
    """
    If fabric_env contains proxy_url, create a SOCKS5 socket connected to
    (target_host, target_port) through that proxy and inject it
    into connect_kwargs.
    """
    proxy_url = fabric_env.get("proxy_url")
    if not proxy_url:
    ...
    # Important: connect to the SSH target THROUGH the proxy
    sock.connect((target_host, target_port))

    connect_kwargs["sock"] = sock
    return sock  # so caller can close it
...
def ssh_connection(ctx, fabric_env, hide_output=True, proxy_settings=None):
    ...
    target_host = fabric_env.get("host")
    target_port = fabric_env.get("port", 22)

    # Create and inject SOCKS proxy socket (if configured)
    proxy_sock = put_proxy(
        fabric_env,
        connect_kwargs,
        target_host,
        target_port)
...
class _RemoteFiles(object):
    ...
    def _upload_ctx(self):
        ...
        if not self.exists(self.remote_proxy_client_path):
            ...
            self.put(
                os.path.join(
                    os.path.dirname(ctx_wrappers.__file__),
                    'ctx-py.py'
                ),
